...
> 状态：**FastAPI 服务 + 同源前端页 + 多阶段 Docker 均已落地**（`src/api/`、`Dockerfile`、`requirements.txt`、`.dockerignore`）；本仓已加 HF Space frontmatter，可一步推到 Hugging Face Docker Space。本节定义 API 契约、统一错误响应、前端形态与容器化/部署。
...
- **依赖**：镜像走精简 `requirements.txt`（从 `environment.yml` pip 段派生、去掉 pytest）；`environment.yml` 仍用于 conda 本地开发。
...
整仓即 Space——README 顶部已带 HF frontmatter（`sdk: docker` / `app_port: 7860`），HF 直接构建本仓 `Dockerfile`：
