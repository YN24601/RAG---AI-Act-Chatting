...
## Serving 与容器化（Day 6-7 规划）

> 状态：**规划**（Day 6-7 实现期落地）。本节定义 API 契约、统一错误响应、前端形态与容器化，作为施工图；代码尚未落地。

把 Day 5 的 `answer_question` 闭环包装成可部署服务：**FastAPI + 多阶段 Docker**。向量库继续留在 **Qdrant Cloud**——容器对数据**无状态**，运行期只注入 API key（无需 `data/`）。

### API 契约

复用 `pydantic v2`（`src/ingestion/schema.py` 起即为此预留），规划目录 `src/api/`，以 `PYTHONPATH=src`（沿用脚本的 sys.path 约定，项目无包安装）运行 `uvicorn api.app:app`。
